...
namespace Discord.Rest
{
    internal static class MessageHelper
    {
        ...
        {
            await client.ApiClient.AddReactionAsync(msg.Channel.Id, msg.Id, EmoteName(emote), options).ConfigureAwait(false);
        }
        ...
        {
            await client.ApiClient.RemoveReactionAsync(msg.Channel.Id, msg.Id, user.Id, EmoteName(emote), options).ConfigureAwait(false);
        }
        ...
        private static string EmoteName(IEmote emote)
        {
            if (emote is Emote)
            {
                var e = (Emote) emote;
        ...
        public static ImmutableArray<ITag> ParseTags(string text, IMessageChannel channel, IGuild guild, IReadOnlyCollection<IUser> userMentions)
        {
            ...
            while (true)
            {
                ...
                else if (Emote.TryParse(content, out emoji))
