    ...
    public partial class DiscordSocketClient : BaseSocketClient, IDiscordClient
    {
        ...
        private async Task SendStatusAsync()
        {
            ...
            if (Activity != null)
            {
                ...
                if (Activity is StreamingGame)
                {
                    var streamGame = (StreamingGame) Activity;
                            ...
                            case "CHANNEL_RECIPIENT_ADD":
                                {
                                    ...
                                    if (State.GetChannel(data.ChannelId) is SocketGroupChannel)
                                    {
                                        var channel = (SocketGroupChannel) State.GetChannel(data.ChannelId);
                            ...
                            case "MESSAGE_CREATE":
                                {
                                    ...
                                    if (State.GetChannel(data.ChannelId) is ISocketMessageChannel)
                                    {
                                        var channel = (ISocketMessageChannel) State.GetChannel(data.ChannelId);
                                        var guild = (channel as SocketGuildChannel)?.Guild;
        ...
        internal ISocketPrivateChannel AddPrivateChannel(API.Channel model, ClientState state)
        {
            ...
            {
                var dm = (SocketDMChannel) channel;
        ...
        internal ISocketPrivateChannel RemovePrivateChannel(ulong id)
        {
            ...
            if (channel != null)
            {
                ...
                {
                    var dm = (SocketDMChannel) channel;
                    dm.Recipient.GlobalUser.DMChannel = null;
                }
        ...
        //IDiscordClient
        async Func<object> IDiscordClient.GetApplicationInfoAsync(RequestOptions options = null)
